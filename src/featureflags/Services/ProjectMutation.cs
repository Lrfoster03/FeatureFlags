    ...
    internal async Task<List<PendingAudit>> PrepareAsync(CancellationToken cancellationToken)
    {
        ...
        foreach (var entry in changed)
        {
            ...
            if (entry.State == EntityState.Modified && entry.Property("Revision").IsModified)
                throw new InvalidOperationException("Revision numbers are managed by the mutation lifecycle.");
        ...
        foreach (var entry in changed.Where(e => e.State == EntityState.Modified))
            entry.Property("Revision").CurrentValue = checked((int)entry.Property("Revision").OriginalValue! + 1);
        return result;
    }
}
